...
public class PagingTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		if (totalCount == 0) {
			...
			// 把当前页号设置成请求参数
			sb.append("<input type=\"hidden\" name=\"").append("page").append("\" value=\"").append(page)
					.append("\"/>\r\n");

			// 输出统计数据
			sb.append("&nbsp;共: <strong>").append(pageCount).append("</strong> 页").append(", <strong>")
					.append(totalCount).append("</strong> 条记录 &nbsp;\r\n");

			// 上一页处理
			if (page == 1) {
				sb.append("<span class=\"disabled\">&laquo;&nbsp;上一页").append("</span>\r\n");
			} else {
				sb.append("<a href=\"javascript:changePage(").append((page - 1))
						.append(")\">&laquo;&nbsp;上一页</a>\r\n");
			}
			...
			if (end < pageCount - 1) {
				sb.append("<a href=\"javascript:changePage(").append(pageCount - 1).append(")\">").append(pageCount - 1)
						.append("</a>\r\n");
			}
			if (end < pageCount) {
				sb.append("<a href=\"javascript:changePage(").append(pageCount).append(")\">").append(pageCount)
						.append("</a>\r\n");
			}
			...
			sb.append("    document.pagingForm.page.value=no;\r\n");
